...
import { addDoc, collection, doc, FieldValue, getDoc, serverTimestamp, orderBy, query, onSnapshot, Timestamp, updateDoc, arrayUnion } from 'firebase/firestore';
...
export const useChat = () => {
  ...
  const sortedChats = historyChats.sort((a, b) => b.createdAt?.seconds - a.createdAt?.seconds);
  setHistoryChats(sortedChats);
    ...
    useEffect(()=>{
      const fetchMessage = async() => {
        try{
          const messagesRef = collection(db, "chat_history", user?.uid, "user_chat_histories", id, "messages");
  ...
  const sendMessage = async (text: string, userData: any) => {
    ...
    if(userData){
      const messagesRef = collection(db, "chat_history", userData.uid, "user_chat_histories", userData.chat_uid, "messages");
